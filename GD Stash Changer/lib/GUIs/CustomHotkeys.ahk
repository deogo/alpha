CustomHotkeysGui_Show()
{
  ...
  Gui,Add,ListView,xm ym w260 h370 Section AltSubmit vCustomHK_LV -Multi gCustomHotkeys_LV_Events
      ,Hotkey|Target|XML_ID|Hotkey_orig
  LV_ModifyCol(1, 100)
;###### Adding hotkeys to list
  CustomHotkeys_Update()
  LV_ModifyCol(3,0) ;hiding XML_ID field
...
;######################################
  ...
  CustomHotkey_ShowItem:
    ...
    if !IsWindowVisible( qcGui["main"] )
      return
    row_num := CustomHotkeys_GetHiddenData( XML_ID, itemHotkey )
    if !row_num
      return
    if( item := qcconf.GetNodeByID( PUM_ID ) )
  ...
  CustomHotkey_Delete:
    ...
    if !( row_num := CustomHotkeys_GetHiddenData( XML_ID, itemHotkey ) )
      return
    if ( item := qcconf.GetNodeByID( XML_ID ) )
    {
      qcconf.ItemSetAttr( XML_ID, "hotkey", "" ) ;removing attribute
      CustomHotkeys_Del( itemHotkey )
      if ( tvid := TVGetIDbyUID( XML_ID ) )
        qcTVHKItems[ tvid ] := 0
  ...
  CustomHotkey_Edit:
    ...
    if ( item := qcconf.GetNodeByID( XML_ID ) )
    {
      if ( itemHotkey != qcconf.ItemGetAttr( XML_ID, "hotkey" ) )  ;comparing hotkey currenly used with the actual in the xml
      {
      ...
      if ( new_hotkey == itemHotkey )
        return
      qcconf.ItemSetAttr( XML_ID, "hotkey", new_hotkey )
...
CustomHotkeys_Update( rebuildMenu = 0 )
{
  ...
  Gui,+OwnDialogs
  LV_Delete()
  for hk,xml_ID in qcCustomHotkeys
    if ( item := qcconf.GetNodeByID( xml_ID ) )
      LV_Add( "", HotkeyToString( hk ), qcconf.ItemGetAttr( xml_ID, "name" ), xml_ID, hk )
...
CustomHotkeys_Change( hk, new_hk,xml_ID = "" )
{
  if (!xml_ID && !( xml_ID := qcCustomHotkeys[ hk ] ))
    return
  CustomHotkeys_Del( hk )
  if (new_hk = "")
    return
  qcCustomHotkeys[ new_hk ] := xml_ID
...
CustomHotkeys_GetHiddenData( byref XML_ID, byref itemHotkey )
{
  Gui,% qcGui[ "main_hotkey_list" ] ":Default"
  Gui,+OwnDialogs
  row_num := LV_GetNext()

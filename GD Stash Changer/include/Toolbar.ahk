...
Toolbar_Add(hGui, Handler, Style="", ImageList="", Pos="") {
    ...
    hCtrl := DllCall("CreateWindowExW" 
             , "uint", 0
             , "Wstr",  "ToolbarWindow32" 
             , "Ptr", 0 
 ...
 ;~ typedef struct {			
  ...
  ;~ BYTE      fsStyle;				char
;~ #ifdef _WIN64
  ;~ BYTE      bReserved[6];		char*6
;~ #else 
;~ #if defined(_WIN32)
  ;~ BYTE      bReserved[2];		char*2
;~ #endif 
;~ #endif 
  ;~ DWORD_PTR dwData;				ptr		8 + ptr
  ;~ INT_PTR   iString;				ptr		8+ 2ptr
;~ } TBBUTTON, *PTBBUTTON, *LPTBBUTTON;
; size = 8 + 3*ptr

	;~ SendMessage,TB_BUTTONSTRUCTSIZE,TBBSize,0,,ahk_id %hCtrl%
	r1 := SendMessage(hCtrl, TB_BUTTONSTRUCTSIZE, 8+3*A_PtrSize, 0 )
	r2 := SendMessage(hCtrl, TB_SETEXTENDEDSTYLE, 0, hExStyle )
	r3 := SendMessage(hCtrl, TB_SETUNICODEFORMAT, A_IsUnicode, 0 )
 ...
 */
Toolbar_AutoSize(hCtrl,rows=1,Align="fit"){
	if align !=
	{
		...
		hParent := DllCall("GetParent", "Ptr", hCtrl,"UPtr")
...
Toolbar_Count(hCtrl, pQ="c") {
	...
	c := SendMessage(hCtrl, TB_BUTTONCOUNT, 0, 0)
	IfEqual, pQ, c, return c

	a := NumGet( Toolbar(hCtrl "aBTN"),0,"Uint" )
...
Toolbar_CommandToIndex( hCtrl, ID ) {
	...
	ret := SendMessage(hCtrl, TB_COMMANDTOINDEX, ID, 0)
	ifEqual, ret, 4294967295, return 0
	return ret + 1
}
...
Toolbar_CheckButton(hCtrl, WhichButton, bCheck=1) {
	...
	SendMessage(hCtrl, TB_CHECKBUTTON, WhichButton, bCheck )
}
...
Toolbar_Define(hCtrl, pQ="") {
	...
	if (pQ="") or (pQ = "a"){
		...
		cnta := NumGet( Toolbar(hCtrl "aBTN"),0,"Uint" )
		loop, %cnta%
			btns .= Toolbar_GetButton(hCtrl, -A_Index) "`n"
...
Toolbar_DeleteButton(hCtrl, Pos=1) {
	...
	TBB_size := 8+3*A_PtrSize
	
	if InStr(Pos, "*") {
		Pos := SubStr(Pos, 2),  aBTN := Toolbar(hCtrl "aBTN"),  cnta := NumGet(aBTN+0,0,"Uint")
		if (Pos > cnta)
			return FALSE
		if (Pos < cnta)
			Toolbar_memmove( aBTN + TBB_size*(Pos-1) +4, aBTN + TBB_size*Pos +4, aBTN + TBB_size*Pos +4)

		NumPut(cnta-1, aBTN+0,0,"Uint")
		return TRUE
	}
...
Toolbar_GetButton(hCtrl, WhichButton, pQ="") {
	...
	if (WhichButton < 0)
		a := Toolbar(hCtrl "aBTN"), aBtn := a + 4,  cnta := NumGet(a+0,0,"Uint"),  WhichButton := -WhichButton,   a := true
 ...
 ;get TBB structure
	VarSetCapacity(TBB, TBB_size), aTBB := &TBB
	if a
		 aTBB := aBtn + WhichButton*TBB_size
	else SendMessage, TB_GETBUTTON, WhichButton, aTBB,,ahk_id %hCtrl%

	id := NumGet(aTBB+0, 4,"Uint")
	IfEqual, pQ, id, return id

 ;check for separator
	if NumGet(aTBB+0, 9, "Char") = 1  {
		loop, % NumGet(TBB,0,"Uint")//10 + 1
			buf .= "-"
		return buf
	}

 ;get caption
	VarSetCapacity( buf, 256, 0 ), sIdx := NumGet(aTBB+0, TBB_size-A_PtrSize,"Uptr")
...
Toolbar_GetMaxSize(hCtrl, ByRef Width, ByRef Height){
	...
	SendMessage(hCtrl,TB_GETMAXSIZE,0,&SIZE)
 ...
 */
 ;размер одной кнопки ебать колотить
 ;~ typedef struct {			
  ;~ int       iBitmap;				uint
  ;~ int       idCommand;			uint
  ;~ BYTE      fsState;				char
...
Toolbar_Insert(hCtrl, Btns, Pos=""){
	...
	if Pos =
		...
		SendMessage, A_IsUnicode ? TB_INSERTBUTTONW : TB_INSERTBUTTONA, Pos+A_Index-2, cBTN + TBB_size*(A_Index-1) ,, ahk_id %hCtrl%
...
Toolbar_SetButton(hCtrl, WhichButton, State="", Width=""){
	...
	static TB_GETSTATE=0x412, TB_GETBUTTON = 0x417
    ...
    if (WhichButton >= 1){
		VarSetCapacity(TBB, TBB_size)
		SendMessage, TB_GETBUTTON, --WhichButton, &TBB,,ahk_id %hCtrl%
		WhichButton := NumGet(&TBB+0, 4, "Uint")
  ...
  ;~ UINT      cbSize;				Uint	0
  ;~ DWORD     dwMask;				uint	4
  ;~ int       idCommand;			uint	8
  ;~ int       iImage;				uint	12
  ;~ BYTE      fsState;				char	16
  ;~ BYTE      fsStyle;				char	17
  ;~ WORD      cx;					short	18
  ;~ DWORD_PTR lParam;				uptr	16+ptr
  ;~ LPTSTR    pszText;				uptr	16+2ptr
  ;~ int       cchText;				uint	16+3ptr
;~ } TBBUTTONINFO, *LPTBBUTTONINFO;			20+3ptr
	size := 16+4*A_PtrSize
	VarSetCapacity(BI, size, 0)
	NumPut(size,		BI, 0,"Uint")
	NumPut(mask,	BI, 4, "Uint")
	NumPut(hState,	BI, 16, "Char")
	NumPut(Width,	BI, 18, "Short")
	WM_USER := 0x0400
	TB_SETBUTTONINFOW := (WM_USER + 64)
	res := SendMessage(hCtrl, TB_SETBUTTONINFOW, WhichButton, &BI)
...
Toolbar_SetImageList(hCtrl, hIL="1S"){
	...
	if hIL is Integer
		ret := SendMessage(hCtrl, TB_SETIMAGELIST, 0, hIL)
	else {
		size := SubStr(hIL,2,1)="L" ? 24:16,  cat := (SubStr(hIL,1,1)-1)*4 + (size=16 ? 0:1)
		SendMessage(hCtrl, TB_SETBITMAPSIZE, 0, (size<<16)+size)
		ret := SendMessage(hCtrl, TB_LOADIMAGES, cat, -1)
	}
...
Toolbar_compileButtons(hCtrl, Btns, ByRef cBTN) {
	static BTNS_SEP=1, BTNS_CHECK =2, BTNS_CHECKGROUP = 6, BTNS_DROPDOWN = 8, BTNS_WHOLEDROPDOWN := 0x0080, BTNS_A=16, BTNS_AUTOSIZE = 16, BTNS_NOPREFIX = 32, BTNS_SHOWTEXT = 64
	static TBSTATE_CHECKED=1, TBSTATE_ENABLED=4, TBSTATE_HIDDEN=8, TBSTATE_DISABLED=0, TBSTATE_WRAP = 0x20, TBSTATE_PRESSED := 0x02, TBSTATE_INDETERMINATE := 0x10, TBSTATE_ELLIPSES := 0x40, TBSTATE_MARKED := 0x80
	...
	if (aBTN = "")
		aBTN := Toolbar_malloc( 50 * TBB_size + 4),  Toolbar(hCtrl "aBTN", aBTN)	 ;if space for array of * buttons isn't reserved and there are definitions of * buttons reserve it for 50 buttons + some more so i can keep some data there...

	StringReplace, _, Btns, `n, , UseErrorLevel
	btnNo := ErrorLevel + 1
	cBTN := Toolbar_malloc( btnNo * TBB_size )

	a := cnt := 0, 	cnta := NumGet(aBTN+0,0,"Uint")		;get number of buttons in the array
	loop, parse, Btns, `n, %A_Space%%A_Tab%	
	{
	 ...
	 ;calculate icon
		if a2 is not Integer					;if user didn't specify icon, use button number as icon index (don't count separators)
			a2 := cnt+cnta+1-sep
		o += TBB_size * (a ? cnta : cnt)				;calculate offset o of this structure in array of TBBUTON structures.
	 ...
	 ;add caption to the string pool
		if (hStyle != BTNS_SEP) {
			...
			sIdx := SendMessage(hCtrl, TB_ADDSTRINGW, 0, &a1) ;returns the new index of the string within the string pool
		} else {
			sIdx := -1
			RegExMatch(A_LoopField,"-+",sep_mtch)
			a2 := (StrLen(sep_mtch)-1)*10 + 1			;if separator, lentgth of the "-" string determines width of the separation. Each - adds 10 pixels.
		}
	 ;TBBUTTON Structure
...
; size = 8 + 3*ptr
		...
		NumPut(a2-1,	o+0, 0, "UInt")			;Zero-based index of the button image. If the button is a separator, determines the width of the separator, in pixels
		NumPut(bid,		o+0, 4, "UInt")			;Command identifier associated with the button
		NumPut(hstate,  o+0, 8, "Char")			;Button state flags
		NumPut(hStyle,  o+0, 9, "Char")			;Button style
		NumPut(0,		o+0, 8+A_PtrSize,"UPtr")				;User data
		NumPut(sIdx,	o+0, 8+2*A_PtrSize, "UPtr")			;Zero-based index of the button string
...
Toolbar_onNotify(Wparam,Lparam,Msg,Hwnd) 
{
	...
	static NM_CLICK=-2, NM_RCLICK=-5, NM_LDOWN=-20, TBN_DROPDOWN=-710, TBN_HOTITEMCHANGE=-713, TBN_ENDDRAG=-702, TBN_BEGINADJUST=-703, TBN_GETBUTTONINFOA=-700, TBN_GETBUTTONINFOW=-720, TBN_QUERYINSERT=-706, TBN_QUERYDELETE=-707, TBN_ENDADJUST=-704, TBN_RESET=-705, TBN_TOOLBARCHANGE=-708, TB_COMMANDTOINDEX=0x419
	static cnt, cnta, cBTN, inDialog, tc=0
	static NM_CUSTOMDRAW := -12
	static NMHDRsize := 3*A_PtrSize

	TBB_size := 8+3*A_PtrSize
;~ Lparam here is the pointer to NMHDR structure
...
;~ } NMHDR;					4 + 2*A_PtrSize
	...
	idfrom := NumGet(Lparam+0,A_PtrSize,"Uptr")
	code := NumGet(Lparam+0, 2*A_PtrSize, "Int")

	if ( code = NM_CUSTOMDRAW && hwndfrom = qcGui[ "main_tv" ] )
		return GUI_Main_TV_CustomDraw( Lparam )
	
	;~ if ( code = NM_CUSTOMDRAW && hwndfrom = qcGui[ "main_lv" ] )
		;~ tooltip % "LV got"

	if (idfrom != MODULEID)
	 ifLess idfrom, 10000, return	;if ahk control, return asap (AHK increments control ID starting from 1. Custom controls use IDs > 10000 as its unlikely that u will use more then 10K ahk controls.
	 else {
		ifEqual, oldNotify, *, SetEnv, oldNotify, % Toolbar("oldNotify")
		if oldNotify !=
			return DllCall(oldNotify, "UPtr", Wparam, "UPtr", Lparam, "uint", Msg, "UPtr", Hwnd)
	 }
	handler := Toolbar(hwndfrom "Handler") 
	ifEqual, handler,, return 
;~ NMTOOLBAR, NMMOUSE, NMTBHOTITEM
	iItem  := code = TBN_HOTITEMCHANGE ? NumGet(lparam+0,NMHDRsize+4,"UInt")
				: code = NM_CLICK ? NumGet(lparam+0,NMHDRsize,"UPtr")
				: code = NM_RCLICK ? NumGet(lparam+0,NMHDRsize,"UPtr")
				: code = NM_LDOWN ? NumGet(lparam+0,NMHDRsize,"UPtr")
				: NumGet(lparam+0,NMHDRsize,"UInt")

	pos := SendMessage(hwndfrom, TB_COMMANDTOINDEX, iItem, 0) + 1
	txt := Toolbar_GetButton( hwndfrom, pos, "c")
	...
	if (code=NM_CLICK) { 		
		IfEqual, pos, 4294967296, return
		if !(A_TickCount - tc)
	 		%handler%(hwndfrom, "click", txt, pos, iItem)
    } 

	if (code=NM_RCLICK)
		ifEqual, pos, 4294967296, return
        else  %handler%(hwndfrom,"rclick", txt, pos, iItem) 

	if (code = TBN_DROPDOWN)
		%handler%(hwndfrom, "menu", txt, pos, iItem)
 
	if (code = TBN_HOTITEMCHANGE) { 
      IfEqual, pos, 4294967296, return  
      return %handler%(hwndfrom, "hot", txt, pos,  iItem) 
   } 

  ;=================== CUSTOMIZATION NOTIFICATIONS =========================== 

	if (code = TBN_BEGINADJUST) { 
		cnta := NumGet( Toolbar(hwndfrom "aBTN") ,0,"Uint") , cnt := Toolbar_getButtonArray(hwndfrom, cBTN), inDialog := true 
		if (cnt=0) && (cnta=0) 
			Msgbox Nothing to customize 
		return 
	} 

	if (code = TBN_GETBUTTONINFOA || code = TBN_GETBUTTONINFOW)   { 
		if (iItem = cnt + cnta)					;iItem is position, not identifier. Win keeps sending incresing numbers until we say "no more" (return 0) 
			return 0 
;~ lparam = NMTOOLBAR
		TBB := lparam + NMHDRsize + 4					;The OS buffer where to put the button structure 
		o := (iItem < cnt) ?  cBTN + TBB_size*iItem : Toolbar( hwndfrom "aBTN") + TBB_size*(iItem-cnt) + 4
		Toolbar_memcpy( TBB, o, TBB_size) ;copy the compiled item into notification struct 
		return 1 
	} 
	...
	if (code=TBN_ENDADJUST) { 
		Toolbar_onEndAdjust(hwndfrom, cBTN, cnt), inDialog := false 
		return %handler%(hwndfrom, "adjust", "", "", "") 
	} 

	;This will fire when user is dragging buttons around with adjustable style
	if (code = TBN_TOOLBARCHANGE) and !inDialog 
		return %handler%(hwndfrom, "change", "", "", "") 
}
...
Toolbar_getButtonArray(hCtrl, ByRef cBtn){
	...
	cBtn := Toolbar_malloc( cnt * TBB_size )
	loop, %cnt%
		SendMessage(hCtrl, TB_GETBUTTON, A_Index-1, cbtn + (A_Index-1)*TBB_size)
	return cnt
}
...
Toolbar_onEndAdjust(hCtrl, cBTN, cnt) {
  ...
  ;check current button changes
	...
	loop, %cnt%
	{
		...
		if NumGet(o+0, 9, "Char") = BTNS_SEP				;skip separators
			continue
	   Toolbar_memcpy( buf + cnta2++*TBB_size +4, o, TBB_size) ;copy the button struct into new array
	}
	Toolbar_mfree(cBTN)	
  ...
  ;check available button changes
	loop, %cnta%
	{
		o := aBTN + TBB_size*(A_Index-1),	id := NumGet(o+0, 4,"Uint")
		SendMessage, TB_COMMANDTOINDEX,id,,,ahk_id %hCtrl%		
		if Errorlevel != 4294967295							;if errorlevel = -1 button isn't on toolbar
			continue
		Toolbar_memcpy(buf + cnta2++*TBB_size +4, o, TBB_size) ;copy the button struct into new array
	...
	NumPut(cnta2, buf+0,0,"Uint")		;save array
...
Toolbar_malloc(pSize){
	...
	return DllCall("VirtualAlloc", "Ptr", 0, "uint", pSize, "uint", MEM_COMMIT, "uint", PAGE_READWRITE)
}

Toolbar_mfree(pAdr) {
	static MEM_RELEASE = 0x8000
	return DllCall("VirtualFree", "Ptr", pAdr, "uint", 0, "uint", MEM_RELEASE)
}

Toolbar_memmove(dst, src, cnt) {
	return DllCall("MSVCRT\memmove", "Ptr", dst, "Ptr", src, "uint", cnt)
}
...
Toolbar_add2Form(hParent, Txt, sOpt ) {
	static f := "Form_Parse"
	
	%f%( sOpt, "x# y# w# h# style IL* g*", x,y,w,h,style,il,handler )
...
;Storage
Toolbar(var_name="", value="GET_VALUE" ) { 
	static store := {}
	cur_value := store[ var_name ]
	if !( value=="GET_VALUE" )
		store[ var_name ] := value
	return cur_value
}

TB_Check(hToolbar,buttonID,checkState)
{
	WM_USER := 0x0400
	TB_CHECKBUTTON := WM_USER + 2
	for i,v in StrSplit(buttonID,"|",A_Space A_Tab)
		if v
			SendMessage(hToolbar, TB_CHECKBUTTON, v, checkState )
	return checkState
}

TB_Hide(hToolbar,buttonID,hideState)
{
	WM_USER := 0x0400
	TB_HIDEBUTTON := WM_USER + 4
	for i,v in StrSplit(buttonID,"|",A_Space A_Tab)
		if v
			SendMessage(hToolbar, TB_HIDEBUTTON, v, hideState )
	return hideState
}

TB_Enable(hToolbar,buttonID,enableState)
{
	WM_USER := 0x0400
	TB_ENABLEBUTTON := WM_USER + 1
	for i,v in StrSplit(buttonID,"|",A_Space A_Tab)
		if v
			SendMessage(hToolbar, TB_ENABLEBUTTON, v, enableState )
	return enableState
}

TB_IsChecked(hToolbar,buttonID)
{
	WM_USER := 0x0400
	TB_ISBUTTONCHECKED := WM_USER + 10
	return SendMessage(hToolbar, TB_ISBUTTONCHECKED, buttonID, 0 )
}

TB_IsPressed(hToolbar,buttonID)
{
	WM_USER := 0x0400
	TB_ISBUTTONPRESSED := WM_USER + 11
	return SendMessage(hToolbar, TB_ISBUTTONPRESSED, buttonID, 0 )
}

TB_Press(hToolbar,buttonID,pressState)
{
	WM_USER := 0x0400
	TB_PRESSBUTTON := WM_USER + 3
	for i,v in StrSplit(buttonID,"|",A_Space A_Tab)
		if v
			SendMessage(hToolbar, TB_PRESSBUTTON, v, pressState )
}

Toolbar_SetButtonImage(hToolbar,buttonID,imageIndex)
{
	WM_USER := 0x0400
	TB_CHANGEBITMAP := WM_USER + 43
	return SendMessage(hToolbar, TB_CHANGEBITMAP, buttonID, imageIndex )
}

Toolbar_SetButtonText(hToolbar,buttonID,NewText)
{
	;~ typedef struct {
  ;~ UINT      cbSize;				Uint	0
  ;~ DWORD     dwMask;				uint	4
  ...
  ;~ LPTSTR    pszText;				uptr	16+2ptr
  ;~ int       cchText;				uint	16+3ptr
;~ } TBBUTTONINFO, *LPTBBUTTONINFO;			16+4*A_PtrSize
...
;~ TB_GETBUTTONINFOW := (WM_USER + 63)
	...
	size := 16+4*A_PtrSize
	VarSetCapacity(tbinfo,size,0)
	numput(size,	tbinfo,0,"Uint")
	numput(TBIF_TEXT,	tbinfo,4,"Uint")
	StrPutVar(NewText, TextBuf, 2 )
	numput(&TextBuf,	tbinfo,16+2*A_PtrSize,"UPtr")
	
	SendMessage(hToolbar, TB_SETBUTTONINFOW, buttonID, &tbinfo )
	Free(TextBuf)
	Free(tbinfo)
	return
}

...
class _ClassMemory
{
    ...
    read(address, type := "UInt", aOffsets*)
    {
        ...
        if !this.aTypeSize.hasKey(type)
            return "", ErrorLevel := -2
        addr := aOffsets.maxIndex() ? this.getAddressFromOffsets(address, aOffsets*) : address
        if DllCall("ReadProcessMemory", "Ptr", this.hProcess, "Ptr", addr, type "*", result, "Int", this.aTypeSize[type], "Ptr", this.pNumberOfBytesRead)
            return result
        return        
    }

...
class PUM extends PUM_base
{
  ...
  Free()
  {
    ...
    ObjSetCapacity( this, "tracelog", 0 )
  }
...
class PUM_Item extends PUM_base
{
  ...
  GetParams()
  {
    static ItemParams := ["issep","name","bold","icon","break","submenu","tcolor","bgcolor","noPrefix","uid","disabled","iconUseHandle"]
    objItemParams := {}
    for each, Param in ItemParams
      objItemParams[Param] := this[Param]
    Return objItemParams
  }
...
{   
  ...
  ShowOverride( x, y, hGui, flags = "" )
  {
    ...
    item := 0
    tpmflags := 0x100
    for i,v in StrSplit( flags, A_Space, A_Space A_Tab )
      if ( val := PUM_Menu._trackConsts[ v ] )
        tpmflags |= val
    isContext := tpmflags & 0x1
    
    if !isContext {
      this.objPUM.gui_override := hGui
      if IsFunc( foo := this.objPum.onshow )
        ret := %foo%( "onshow", this )
      if ( ret = 0 ) {
        this.objPUM.gui_override := 0
        return
      }
    ...
    itemID := pumAPI._TrackPopupMenuEx( this.handle, tpmflags, x, y, hGui )
    if !isContext
  ...
  Show( x = 0, y = 0, flags = "" )
  {
    if !this.alive
      return 0
    if this.objPUM.gui_override
      return this.ShowOverride(x,y,this.objPUM.gui_override,flags)
    item := 0
    tpmflags := 0x100
    for i,v in StrSplit( flags, A_Space, A_Space A_Tab )
      if ( val := PUM_Menu._trackConsts[ v ] )
...
{
  static last_pos, last_time, last_hmenu, last_code
  charCode := wParam & 0xffff
  type := wParam >> 16
  hMenu := lParam
  itemsList := pumAPI._GetMenuItems( hMenu )
  oMenu := pumAPI._GetMenuFromHandle( hMenu )
  if isFunc( foo := oMenu.objPUM.onchar ){
    %foo%( "onchar", oMenu, charCode )
    return 0
  }
  mnemMode := oMenu.objPUM.mnemonicCmd
  fClean := False
  if last_time
  {
    cur_time := A_Now
    cur_time -= last_time, Seconds
    if( cur_time > 2 )
      fClean := True
  }
  if( last_hmenu != hMenu )
    fClean := True
  if( last_code != charCode )
    fClean := True
  if fClean
  {
    last_pos := 0
  ...
  if( mnemMode = "run" ) ; searching for ampersanded item to run first
  {
    for i,item in itemsList
      if ( item.hotCharCode == charCode && item.fMnemAmp = 1 )
        itemPos := i-1
  }
  ...
  if( last_pos && itemPos = "" ) ; sets first mathing item in menu in case we are at the end of menu already
    itemPos := firstPos
  if ( itemPos != "" )
  {
    last_time := A_Now
    ...
    last_pos := itemPos
    last_code := charCode
    ret_mode := mnemMode = "select" ? 3 : 2
    return ( ret_mode << 16 ) | itemPos
  }
  last_hmenu := 0 ; to make sure clean will be performed at next call
  return 0
}
...
PUM_OnDraw( wParam, lParam, msg, hwnd )
{
  ...
  fDebug := item.menu.objPUM.debug
  objPUM := item.menu.objPUM
  ...
  if fDebug
  {
    sAction := itemAction = pumAPI.ODA_FOCUS ? "ODA_FOCUS"
              : itemAction = pumAPI.ODA_DRAWENTIRE ? "ODA_DRAWENTIRE"
              : itemAction = pumAPI.ODA_SELECT ? "ODA_SELECT"
              : itemAction
    sItemState := "" . ( itemState & pumAPI.ODS_SELECTED ? ".SELECTED" : "" )
                  . ( itemState & pumAPI.ODS_GRAYED ? ".GRAYED" : "" )
                  . ( itemState & pumAPI.ODS_DISABLED ? ".DISABLED" : "" )
                  . ( itemState & pumAPI.ODS_CHECKED ? ".CHECKED" : "" )
                  . ( itemState & pumAPI.ODS_FOCUS ? ".FOCUS" : "" )
                  . ( itemState & pumAPI.ODS_DEFAULT ? ".DEFAULT" : "" )
                  . ( itemState & pumAPI.ODS_COMBOBOXEDIT ? ".COMBOBOXEDIT" : "" )
                  . ( itemState & pumAPI.ODS_HOTLIGHT ? ".HOTLIGHT" : "" )
                  . ( itemState & pumAPI.ODS_INACTIVE ? ".INACTIVE" : "" )
                  . ( itemState & pumAPI.ODS_NOACCEL ? ".NOACCEL" : "" )
                  . ( itemState & pumAPI.ODS_NOFOCUSRECT ? ".NOFOCUSRECT" : "" )
    objPUM.PUMlog( "'" item.name "': action." sAction " state" sItemState " sel." isItemSelected " tcolor." tcolor " bgcolor." bgcolor
                    . " selTColor." selTColor " selBGColor." selBGColor )
  }
  ...
  else ;ODA_DRAWENTIRE | ODA_SELECT
  {
    if item.issep
    {
      ...
      if fDebug
        objPUM.PUMlog( "'" item.name "': DrawEdge (sep)" )
    }
    else
    {
      if (   ( itemAction = pumAPI.ODA_SELECT && selMethod = "fill" ) 
          || ( itemAction = pumAPI.ODA_DRAWENTIRE ) )
      {
        clr := isItemSelected && selMethod = "fill" ? selBGColor : bgcolor
        pumAPI.FillRect( hDC, pRect, clr )
        if fDebug
          objPUM.PUMlog( "'" item.name "': FillRect with color " clr )
      }
      if ( selMethod = "frame" )
      {
        clr := isItemSelected ? tcolor : bgcolor
        loop, % item.menu.objPUM.frameWidth
        {
          infNum := A_Index - 1
          pumAPI.SetRect( sepRect
                        , left + infNum
                        , top + infNum
                        , right - infNum
                        , bottom - infNum )
          pumAPI.InflateRect( &sepRect, 0, 0 )
          pumAPI.FrameRect( hDC, &sepRect, clr )
        }
        if fDebug
          objPUM.PUMlog( "'" item.name "': frame drawn with color " clr )
      }

      ;drawing text
      if ( !itemNoText 
        && ( ( itemAction = pumAPI.ODA_SELECT && selMethod = "fill" ) 
          || ( itemAction = pumAPI.ODA_DRAWENTIRE ) ) )
      {
        tClr := isItemSelected && selMethod = "fill" ? selTColor : tcolor
        bClr := isItemSelected && selMethod = "fill" ? selBGColor : bgcolor
        ...
        if fDebug
          objPUM.PUMlog( "'" item.name "': text drawn with colors t." tClr " bg." bClr )
      }
      ;drawing icon
              ...
              || itemAction = pumAPI.ODA_DRAWENTIRE ) )
      {
        pumAPI.DrawIconEx( hDC
                    ...
                    , item.GetIconHandle() )
        if fDebug
          objPUM.PUMlog( "'" item.name "': icon drawn" )
      }
               ...
               || itemAction = pumAPI.ODA_DRAWENTIRE ) )
      {
        ...
        clr := isItemSelected && selMethod = "fill" ? selTColor : tcolor
        ...
        if fDebug
          objPUM.PUMlog( "'" item.name "': submenu arrow drawn" )
      }
    }
  }

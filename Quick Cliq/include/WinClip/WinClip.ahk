class WinClip extends WinClip_base
{
  ...
  __from_cb_msg()
  {
    if !WinClipAPI.OpenClipboard()
      return 0
    allnum := WinClipAPI.CountClipboardFormats()
    s := "Amount of formats currently on clipboard: " allnum "`n"
    nextformat := 0
    while ( nextformat := WinClipAPI.EnumClipboardFormats( nextformat ) )
    {
      fmtname := this._getFormatName( nextformat )
      fmtsize := "???"
      if ( dataHandle := WinClipAPI.GetClipboardData( nextformat ) )
      {
        ...
        WinClipAPI.GlobalUnlock( dataHandle )
      }
      s .= nextformat ": " fmtname " - size " fmtsize "`n"
    }
    WinClipAPI.CloseClipboard()
    return s
  }
...
<!--EndFragment-->
  ...
  _setFiles( ByRef clipData, clipSize, files, append = 0, isCut = 0 )
  {
    ...
    objFiles := StrSplit( prevList . files, "`n", A_Space A_Tab )
